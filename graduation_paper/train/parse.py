...
def parse_sampling_log(fnum, log_path, mode, proportion):
    ...
    for line in lines:
        if line.find('TrainLog') != -1:
            fid, running_time, feature_vector = parse_sampling_log_data(line)
            for i in range(fnum):
                total_datasets[i].append(feature_vector)
                total_labels[i].append(running_time)
    log_file.close()
...
def parse_sampling_log_data(line):
    data = line[line.find('TrainLog'):]
    data_array = data.split(';')
    fid = int(data_array[0][14:])
    running_time = float(data_array[1])
    ...
    next_compute_size = int(data_array[8])
    step = int(data_array[9])
    sampling_number = int(data_array[10])
    ...
    feature_vector.append(next_compute_size)
    feature_vector.append(step)
    feature_vector.append(sampling_number)

    return fid, running_time, feature_vector

def parse_sssp_log(fnum, log_path, total_vertex_num, L, mode, proportion):
    total_datasets = [None] * fnum
    total_labels = [None] * fnum
    ...
    for line in lines:
        if line.find('TrainLog') != -1:
            fid, running_time, feature_vector = parse_sssp_log_data(line, total_vertex_num, L)
...
def parse_sssp_log_data(line, total_vertex_num, L):
...
def parse_pagerankx_log(fnum, log_path, mode, proportion):
    total_datasets = [None] * fnum
    total_labels = [None] * fnum
    ...
    for line in lines:
        if line.find('TrainLog') != -1:
            fid, running_time, feature_vector = parse_pagerankx_log_data(line)
            if total_datasets[fid] == None:
                total_datasets[fid] = []
                total_labels[fid] = []
            total_datasets[fid].append(feature_vector)
...
def parse_pagerankx_log_data(line):
    data = line[line.find('TrainLog'):]
    data_array = data.split(';')
    fid = int(data_array[0][14:])
    running_time = float(data_array[1]) * 1000
...
def parse_wcc_hashmin_log(fnum, log_path, total_vertex_num, L, mode, proportion):
    total_datasets = [None] * fnum
    total_labels = [None] * fnum
    ...
    for line in lines:
        if line.find('TrainLog') != -1:
            fid, running_time, feature_vector = parse_wcc_hashmin_log_data(line, total_vertex_num, L)
            if total_datasets[fid] == None:
                total_datasets[fid] = []
                total_labels[fid] = []
            total_datasets[fid].append(feature_vector)
...
def parse_wcc_hashmin_log_data(line, total_vertex_num, L):
    interval = math.ceil(total_vertex_num / L);
